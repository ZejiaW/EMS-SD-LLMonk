            ...
            if first_context_decoding:
                ...
                if comm.is_pipeline_group_first():
                    ...
                    if ENABLE_LLMA and not (token_nums_per_sample_max == 1 and token_nums_per_sample_min == 1):
                        ...
                        for idx in range(batch_size):
                            input_tokens_list.append(output_token_ids[global_position_idx:draft_sequence_lengths[idx].item(), idx])
                            position_ids_list.append(global_position_ids[global_position_idx:draft_sequence_lengths[idx].item(), idx])
                            assert token_nums_per_sample[idx] == draft_sequence_lengths[idx].item() - global_position_idx
                ...
                if ENABLE_LLMA and not (token_nums_per_sample_max == 1 and token_nums_per_sample_min == 1):
                    ...
                    for idx, token_nums in enumerate(token_nums_per_sample):
                        ...
                        if token_nums_item == 0:
                            continue
                        predict_tokens_idx = predict_tokens[batch_offset:batch_offset+token_nums_item]
                        if accept_lengths[idx] == max_accept_length:
                            output_token_ids[global_position_idx+max_accept_length, idx] = predict_tokens_idx[accept_lengths[idx]-1]
                            global_position_ids[global_position_idx+max_accept_length, idx] = input_lengths[idx]+generated_tokens[idx]+accept_lengths[idx]-1
                        else:
                            masked_tokens[idx, global_position_idx+accept_lengths[idx]:global_position_idx+max_accept_length] = True
                            output_token_ids[global_position_idx+accept_lengths[idx]:global_position_idx+max_accept_length+1, idx] = predict_tokens_idx[accept_lengths[idx]-1]
