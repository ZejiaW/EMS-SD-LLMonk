    ...
    for step in range(max_input_length, max_seq_length):
        ...
        for ite in range(num_local_batches):
            ...
            if first_context_decoding:
                ...
                if comm.is_pipeline_group_first():
                    ...
                    if ENABLE_LLMA and not (token_nums_per_sample_max == 1 and token_nums_per_sample_min == 1):
                        ...
                        for idx in range(batch_size):
                            input_tokens_list.append(output_token_ids[sequence_lengths[idx].item():draft_sequence_lengths[idx].item(), idx])
                            position_ids_list.append(global_position_ids[sequence_lengths[idx].item():draft_sequence_lengths[idx].item()])
                            assert token_nums_per_sample[idx] == draft_sequence_lengths[idx].item() - sequence_lengths[idx].item()
            ...
            if comm.is_pipeline_group_last():
                ...
                # argmax
                predict_tokens = logits[:,:vocab_size].argmax(-1)
                if ENABLE_LLMA and not (token_nums_per_sample_max == 1 and token_nums_per_sample_min == 1):
                    # verify
                    batch_offset = 0
