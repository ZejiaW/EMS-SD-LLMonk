...
def gpt_LLMA(
    ...
    max_seq_length = max_input_length + gen_length + copy_length*2 + 1
    ...
    global_position_ids = torch.arange(max_seq_length, dtype=torch.int32, device=device)
        ...
        for ite in range(num_local_batches):
            ...
            if ENABLE_LLMA and not first_context_decoding:
                for idx in range(batch_size):
                    ...
                    copy_index = lookup_table[idx].get(tuple(output_token_ids[cur_length-check_length:cur_length, idx].tolist()), None)
            ...
            token_nums_per_sample_min = token_nums_per_sample.min()
            token_nums_per_sample_max = token_nums_per_sample.max()
            ...
            if first_context_decoding:
                ...
                if comm.is_pipeline_group_first():
                    ...
                    if ENABLE_LLMA and not (token_nums_per_sample_max == 1 and token_nums_per_sample_min == 1):
                        ...
                        for idx in range(batch_size):
                            input_tokens_list.append(output_token_ids[sequence_lengths[idx].item():sequence_lengths[idx].item()+token_nums_per_sample_max, idx])
                            position_ids_list.append(global_position_ids[sequence_lengths[idx].item():sequence_lengths[idx].item()+token_nums_per_sample_max])
                            token_nums_per_sample[idx] = token_nums_per_sample_max
                        token_nums_per_sample_min = token_nums_per_sample_max
                        ...
                        position_ids = sequence_lengths.clone()
            ...
            if comm.is_pipeline_group_last():
                ...
                if ENABLE_LLMA and not (token_nums_per_sample_max == 1 and token_nums_per_sample_min == 1):
                    ...
                    output_token_ids[sequence_lengths.cpu().numpy()+1, bbidx] = predict_tokens
                    sequence_lengths += (1-finished.int())

                    finished = finished  | (predict_tokens==eos_token_id)
                    finished = finished  | (sequence_lengths+1-input_lengths>=gen_length)
                torch.cuda.synchronize()
