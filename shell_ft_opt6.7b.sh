...
do
    ...
    do
        ...
        mpirun -n $tensor_para python ../examples/pytorch/gpt/opt_summarization.py --hf_model_name ../models/huggingface-models/hf_model/opt-$model/ --ft_model_location ../models/huggingface-models/c-model/opt-$model/ --data_type fp16 --verbose --weights_data_type fp16 --use_gpt_decoder_ops --summarize --tensor_para_size $tensor_para --max_batch_size $bs --name $name --num_samples $num_samples --output_len $output_len --save_root $save_root > $save_root/logs/$name.log
        # LLMA
        ...
        do
            ...
            do
                ...
                mpirun -n $tensor_para python ../examples/pytorch/gpt/opt_summarization.py --hf_model_name ../models/huggingface-models/hf_model/opt-$model/ --ft_model_location ../models/huggingface-models/c-model/opt-$model/ --data_type fp16 --verbose --weights_data_type fp16 --use_gpt_decoder_ops --summarize --tensor_para_size $tensor_para --max_batch_size $bs --name $name --method $method --num_samples $num_samples --output_len $output_len --draft_ft_model_location ../models/huggingface-models/c-model/opt-$draft_model/ --draft_hf_model_name ../models/huggingface-models/hf_model/opt-$draft_model/ --draft_steps_max $draft_steps_max --save_root $save_root > $save_root/logs/$name.log
            done
        done
        
        # bs > 1
        for bs in 24 20 16 12 8 4 2
        do
            # baseline
            method=baseline
            name=$model-$num_samples-output_len$output_len-$method-bs$bs-repeat$r
